...
class RCRRLearner(core.Learner):
    ...
    def __init__(self,
        ...
        self._save_every_n_hours = save_every_n_hours
        if checkpoint:
            self._checkpointer = tf2_savers.Checkpointer(
                directory='./models/crr_learner',
                ...
                keep_checkpoint_every_n_hours=self._save_every_n_hours
                )
    ...
    def _step(self, sample: reverb.ReplaySample) -> Dict[str, tf.Tensor]:
        ...
        justtogetshapes = tf.shape(actions)
        per_device_batch_size=justtogetshapes[1]
        ...
        with tf.GradientTape(persistent=True) as tape:
            for t in range(1, self._sequence_length):
                o_tm1 = tree.map_structure(
                    operator.itemgetter(t - 1), observations)
